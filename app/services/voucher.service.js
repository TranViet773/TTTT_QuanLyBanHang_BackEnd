...
const createVoucher = async (data) => {
    ...
    const { VOUCHER_CODE, TYPE, VALUE, APPLY_SCOPE, CREATE_BY, MAX_DISCOUNT, QUANTITY, START_DATE, END_DATE } = data;
    const now = new Date ();
    if (VALUE <= 0) {
        return {
    ...
    try {
        ...
        throw new Error("Voucher code đã tồn tại hoặc có lỗi trong quá trình tạo voucher");
    }


};

const getAllVouchers = async ({ page = 1, limit = 10, search = "", isActive, type, minUse, maxUse, applyScope, startDate, endDate }) => {
    try {
...
const getVoucherById = async (voucherCode) => {
    try {
        ...
        if (!voucher) {
            return {
                error: `không có vouvher với mã '${voucherCode}'`,
            };
        }
        return {
            voucher
